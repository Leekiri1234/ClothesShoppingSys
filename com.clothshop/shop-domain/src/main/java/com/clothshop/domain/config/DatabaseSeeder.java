...
public class DatabaseSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        if (roleRepository.count() > 0) {
            log.info("Database already seeded. Running extended seed checks...");
            seedExtendedDataIfNeeded();
            return;
        }
    ...
    private void seedExtendedDataIfNeeded() {
        ensureAdditionalStaffAccounts();
        ensureAdditionalCustomers();
        ensureAdditionalChildCategories();
        ensureAdditionalProducts();
        ensureAdditionalOrders();
        ensureFeedbackSeeds();
        ensureAdminNotifications();
    }

    private void ensureAdditionalChildCategories() {
        createChildCategoryIfMissing("men-fashion", "Shirts", "men-shirts");
        createChildCategoryIfMissing("men-fashion", "Pants", "men-pants");
        createChildCategoryIfMissing("women-fashion", "Dresses", "women-dresses");
        createChildCategoryIfMissing("women-fashion", "Skirts", "women-skirts");
        createChildCategoryIfMissing("accessories", "Belts", "accessories-belts");
        createChildCategoryIfMissing("accessories", "Jewelry", "accessories-jewelry");
        createChildCategoryIfMissing("bags", "Mini Bags", "bags-mini");
        createChildCategoryIfMissing("bags", "Office Bags", "bags-office");
    }

    private void createChildCategoryIfMissing(String parentSlug, String childName, String childSlug) {
        if (categoryRepository.existsByCategorySlug(childSlug)) {
            return;
        }

        Category parent = categoryRepository.findByCategorySlug(parentSlug).orElse(null);
        if (parent == null) {
            return;
        }

        Category child = createCategory(childName, childSlug, CategoryStatus.ACTIVE);
        child.setParent(parent);
        categoryRepository.save(child);
    }

    private void ensureAdditionalStaffAccounts() {
        createStaffIfMissing("marketing2", "marketing@123", "marketing2@clothshop.com",
            "Pham Thi Marketing", "0911222333", StaffRole.MARKETING_STAFF);
        createStaffIfMissing("sale2", "sale@123", "sale2@clothshop.com",
            "Do Van Sales", "0911444555", StaffRole.SALE_PRODUCT_STAFF);
        createStaffIfMissing("support2", "support@123", "support2@clothshop.com",
            "Bui Thi Support", "0911666777", StaffRole.CUSTOMER_SERVICE);
    }

    private void ensureAdditionalCustomers() {
        createCustomerIfMissing("customer4", "customer@123", "customer4@email.com",
            "Hoang Minh D", "0912000004", "12 Nguyen Dinh Chieu, District 3");
        createCustomerIfMissing("customer5", "customer@123", "customer5@email.com",
            "Pham My E", "0912000005", "78 Vo Van Tan, District 1");
        createCustomerIfMissing("customer6", "customer@123", "customer6@email.com",
            "Tran Khai F", "0912000006", "26 Dien Bien Phu, Binh Thanh");
    }

    private void ensureAdditionalProducts() {
        if (productRepository.count() >= 16) {
            return;
        }

        Category menFashion = categoryRepository.findByCategorySlug("men-fashion").orElse(null);
        Category womenFashion = categoryRepository.findByCategorySlug("women-fashion").orElse(null);
        Category bags = categoryRepository.findByCategorySlug("bags").orElse(null);
        if (menFashion == null || womenFashion == null || bags == null) {
            return;
        }

        seedProductIfMissing(menFashion, "Oxford Linen Shirt", "oxford-linen-shirt", "Soft linen shirt for office-casual",
            new BigDecimal("459000"), "White", "M", 34, "/images/admin/product-shirt.svg");
        seedProductIfMissing(womenFashion, "Pleated Midi Skirt", "pleated-midi-skirt", "Pleated skirt for modern wardrobe",
            new BigDecimal("529000"), "Beige", "S", 28, "/images/admin/product-dress.svg");
        seedProductIfMissing(bags, "Everyday Tote Bag", "everyday-tote-bag", "Roomy tote for daily essentials",
            new BigDecimal("389000"), "Brown", "One Size", 46, "/images/admin/product-bag.svg");
    }

    private void seedProductIfMissing(Category category,
                                      String name,
                                      String slug,
                                      String description,
                                      BigDecimal price,
                                      ...
                                      int stock,
                                      String imageUrl) {
        if (productRepository.existsByProductSlug(slug)) {
            return;
        }

        Product product = createProduct(category, name, slug, description, price, ProductStatus.ACTIVE);
        productRepository.save(product);

        ProductVariant variantOne = createVariant(product, slug.toUpperCase().replace('-', '_') + "_A", color, size, stock, price, imageUrl);
        ProductVariant variantTwo = createVariant(product, slug.toUpperCase().replace('-', '_') + "_B", color, "L", Math.max(8, stock / 2), price, imageUrl);
        productVariantRepository.save(variantOne);
        productVariantRepository.save(variantTwo);

        ProductImage image = createProductImage(product, imageUrl, 1, true);
        productImageRepository.save(image);
    }

    private void ensureAdditionalOrders() {
        if (orderRepository.count() >= 15 || orderRepository.existsByOrderInvoiceStartingWith("DEMO-2026")) {
            return;
        }

        List<Customer> customers = customerRepository.findAll();
        List<ProductVariant> variants = productVariantRepository.findAll();
        if (customers.isEmpty() || variants.size() < 10) {
            return;
        }

        Customer customerA = customers.get(0);
        Customer customerB = customers.size() > 1 ? customers.get(1) : customers.get(0);
        Customer customerC = customers.size() > 2 ? customers.get(2) : customers.get(0);
        ...
        createOrderWithVoucher(
            ...
            PaymentMethod.COD,
            OrderStatus.PENDING,
            List.of(new ItemSpec(variants.get(0), 1), new ItemSpec(variants.get(2), 1)),
            null,
            "Demo order pending"
        );
        ...
        createOrderWithVoucher(
            ...
            List.of(new ItemSpec(variants.get(1), 2)),
            null,
        ...
        createOrderWithVoucher(
            ...
            List.of(new ItemSpec(variants.get(3), 1), new ItemSpec(variants.get(4), 1)),
            null,
            "Demo order shipping"
        );
        ...
        createOrderWithVoucher(
            ...
            List.of(new ItemSpec(variants.get(5), 1), new ItemSpec(variants.get(6), 2)),
            null,
        ...
        createOrderWithVoucher(
            ...
            List.of(new ItemSpec(variants.get(7), 1), new ItemSpec(variants.get(8), 1)),
            null,
        ...
        createOrderWithVoucher(
            ...
            List.of(new ItemSpec(variants.get(9), 1)),
            null,
        ...
        createOrderWithVoucher(
            ...
            PaymentMethod.MOMO,
            OrderStatus.DELIVERED,
            List.of(new ItemSpec(variants.get(2), 1), new ItemSpec(variants.get(4), 1)),
            null,
            "Demo order delivered second batch"
        );
        ...
        createOrderWithVoucher(
            ...
            null,
            "Demo order completed second batch"
        );
        ...
        createOrderWithVoucher(
            ...
            List.of(new ItemSpec(variants.get(1), 1)),
            null,
            "Demo order returned"
        );
        ...
        createOrderWithVoucher(
            ...
            PaymentMethod.BANK_TRANSFER,
            OrderStatus.CONFIRMED,
            List.of(new ItemSpec(variants.get(3), 1), new ItemSpec(variants.get(8), 1)),
            null,
            "Demo order confirmed second batch"
        );
    }

    private void ensureFeedbackSeeds() {
        long pending = productFeedbackRepository.countByFeedbackStatus("PENDING");
        long approved = productFeedbackRepository.countByFeedbackStatus("APPROVED");
        ...
        if (pending == 0) {
            ProductFeedback pendingFeedback = ProductFeedback.builder()
                .product(products.get(0))
                .customer(customer)
                .order(latestOrder)
                .rating(4)
                .comment("San pham dep, mong duoc duyet review som")
                .feedbackStatus("PENDING")
                .createdBy("SYSTEM")
                .build();
            productFeedbackRepository.save(pendingFeedback);
        }

        if (approved == 0) {
            Product targetProduct = products.size() > 1 ? products.get(1) : products.get(0);
            ProductFeedback approvedFeedback = ProductFeedback.builder()
                .product(targetProduct)
                .customer(customer)
                .order(latestOrder)
                .rating(5)
                .comment("Chat lieu mem, dang form dep")
                .feedbackStatus("APPROVED")
                .moderatedBy("SYSTEM")
                .moderatedAt(LocalDateTime.now().minusDays(1))
                .createdBy("SYSTEM")
                .build();
            productFeedbackRepository.save(approvedFeedback);
        }
    }

    private void ensureAdminNotifications() {
        createNotificationIfMissing("[SYSTEM] Don hang moi can duyet",
            "Co don hang moi dang o trang thai cho xac nhan.",
            NotificationType.ORDER_UPDATE,
            "ALL_CUSTOMERS");
        createNotificationIfMissing("[SYSTEM] Co feedback cho duyet",
            "Bo phan CSKH vui long kiem duyet feedback moi.",
            NotificationType.SYSTEM,
            "ALL_CUSTOMERS");
    }

    private void createNotificationIfMissing(String title,
                                             String content,
        ...
        List<Account> staffAccounts = accountRepository.findByAccountTypeAndIsActiveTrue(AccountType.STAFF);
        staffAccounts.stream().limit(3).forEach(account -> {
    ...
    private void createStaffIfMissing(String username,
                                      String rawPassword,
                                      String email,
                                      String fullName,
                                      String phone,
        ...
        Role role = roleRepository.findByStaffRole(roleType)
            .orElseThrow(() -> new RuntimeException(roleType + " role not found"));

        Account account = createAccount(username, rawPassword, email, AccountType.STAFF, AccountStatus.ACTIVE);
    ...
    private void createCustomerIfMissing(String username,
                                         String rawPassword,
                                         String email,
                                         String fullName,
                                         String phone,
        ...
        Account account = createAccount(username, rawPassword, email, AccountType.CUSTOMER, AccountStatus.ACTIVE);
        accountRepository.save(account);
    ...
    private void createOrderWithVoucher(String invoice, Customer customer, PaymentMethod paymentMethod,
        ...
        List<OrderItem> orderItems = items.stream()
            .map(item -> OrderItem.builder()
                ...
                .finalPrice(item.variant().getRetailPrice()
